...
const getIcon = (iconString: string) => {
  ...
  // 2. Check Lucide icons dynamically
  // Convert kebab-case to PascalCase for Lucide lookup (e.g., 'arrow-right' -> 'ArrowRight')
  const pascalCaseIcon = iconString
    .split('-')
    .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
    .join('');

  // Cast LucideIcons to any to access by string key
  const LucideIcon = (LucideIcons as any)[pascalCaseIcon];

  if (LucideIcon) {
    return <LucideIcon className="w-7 h-7" strokeWidth={1.5} />;
  }
